...
public class BadItem {
	...
	public BadItem(Game game) {		
		this.game = game;

		w = game.baditemImage.getWidth();
		h = game.baditemImage.getHeight();
	...
	public void spawn() {				
		...
		Random r = new Random();
		int i1 = r.nextInt(game.MAX_BAD_IMAGES);
		game.baditemImage = game.baditemImages[i1];
		h = game.baditemImage.getHeight();
	...
	public void update() {
		...
		if (game.curScore > 4000) {
			...
			y =  (float) (game.groundY - h - (200-55) - ((200)/2)*Math.sin(freq));
			rect.top = y;
			rect.bottom = y + h;
		}
		//
	...
	public void draw(Canvas canvas) {
		canvas.drawBitmap(game.baditemImage, rect.left, rect.top, game.clearPaint);
	}
